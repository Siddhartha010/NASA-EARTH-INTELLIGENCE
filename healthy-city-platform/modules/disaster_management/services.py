...
class EarlyWarningService:
    def __init__(self):
        # simple in-memory store for user-submitted alerts
        # In production this should be replaced with a persistent DB
        self.user_alerts = []
    ...
    # --- User-reported alerts persistence helpers ---
    async def add_user_alert(self, alert_data: Dict) -> Dict:
        """Add a user-submitted alert to the in-memory store and return the saved alert."""
        alert_id = f"user_alert_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{len(self.user_alerts)+1}"
        saved = {
            "alert_id": alert_id,
            "reporter": alert_data.get("reporter", "anonymous"),
            "disaster_type": alert_data.get("disaster_type", "general"),
            "severity": alert_data.get("severity", "warning"),
            "message": alert_data.get("message", "User reported alert"),
        ...
        self.user_alerts.insert(0, saved)
        return saved

    async def get_user_alerts(self) -> List[Dict]:
        """Return all user-submitted alerts."""
        return list(self.user_alerts)